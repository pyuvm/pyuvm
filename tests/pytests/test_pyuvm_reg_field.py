...
##############################################################################
## TESTS pyuvm_reg_field
##############################################################################
@pytest.mark.test_reg_field_get_name
def test_reg_field_get_name():
    field_with_explicit_name = pyuvm_reg_field("some_field")
    assert field_with_explicit_name.get_name() == "some_field"
    field_with_implicit_name = pyuvm_reg_field()
    assert field_with_implicit_name.get_name() == 'pyuvm_reg_field'

@pytest.mark.test_reg_field_configure
def test_reg_field_configure():
    field = pyuvm_reg_field()
    parent = pyuvm_reg()
...
def test_reg_field_is_volatile():
    field = pyuvm_reg_field("FIELD_volatile")
    field.configure(pyuvm_reg(), 8, 16, 'RW', True, 15)
    field.field_lock()
    assert field.is_volatile()
    field.configure(pyuvm_reg(), 8, 16, 'RW', False, 15)
    field.field_lock()
...
def test_reg_field_all_access():
    for acs in ["RO","RW","RC","RS","WC","WS","W1C","W1S","W1T","W0C","W0S","W0T","WRC","WRS","W1SRC","W1CRS","W0SRC","W0CRS","WSRC","WCRS","WO","WOC","WOS","W1","WO1"]:
        field = pyuvm_reg_field("FIELD_"+acs)
        field.configure(pyuvm_reg(), 8, 16, acs, True, 15)
        field.field_lock()
...
def test_reg_field_lsb_pos():
    field = pyuvm_reg_field("FIELD_lsb_pos")
    field.configure(pyuvm_reg(), 8, 16, 'RW', True, 15)
    field.field_lock()
    assert field.get_lsb_pos() == 16
...
def test_reg_field_field_predict_read_set():
    ...
    for acs in ["RS","WRS","WCRS","W1CRS","W0CRS"]:
        ...
        assert field.get_response() == pyuvm_resp_t.PASS_RESP, "Failing as default status is not PASS_RESP is {}".format(field.get_response())
...
def test_reg_field_field_predict_write_set():
    ...
    for acs in ["WSRC","WOS","WS","W0S","W1SRC","W0SRC","W1S"]:
        field.configure(pyuvm_reg(), 8, 16, acs, True, 0)
        field.field_lock()
        field.reset()
        local_rand_el = randint(1,(2**field.get_n_bits()-1))
        field.set_prediction(predict_t.PREDICT_DIRECT)
        field.field_predict(local_rand_el,path_t.FRONTDOOR,access_e.PYUVM_WRITE)
...
def test_reg_field_field_predict_NO_ACCESS():
    ...
    field.configure(pyuvm_reg(), 8, 16, "NO_ACCESS", True, randint(1,2**8-1))
    field.field_lock()
    field.reset()
    field.set_prediction(predict_t.PREDICT_DIRECT)
...
def test_reg_field_field_predict_status_error_on_write():
    ...
    for acs in ["RO","RW","RC","RS"]:
        field.configure(pyuvm_reg(), 8, 16, acs, True, randint(1,2**8-1))
        field.field_lock()
        ...
        field.set_prediction(predict_t.PREDICT_DIRECT)
        field.field_predict(randint(1,2**field.get_n_bits()),path_t.FRONTDOOR,access_e.PYUVM_WRITE)
        assert field.get_response() == pyuvm_resp_t.ERROR_RESP, "Failing for access access: {} where UVM_READ is issued response is: {}".format(acs,field.get_response().name) 
