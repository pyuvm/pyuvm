...
def test_reg_field_all_access():
    for acs in [
        ...
        field.field_lock()
        assert field.get_access() == acs, f"Access value {acs} not in the list"
...
def test_reg_field_reset():
    ...
    field.reset()
    assert field.get_reset() == 15, (
        f"after reset internal reset value doesn't match {field.get_reset()}"
    )
    assert field.get_value() == 15, (
        f"after reset internal mirrored value doesn't match {field.get_value()}"
    )
...
def test_reg_field_field_predict_read_set():
    ...
    for acs in ["RS", "WRS", "WCRS", "W1CRS", "W0CRS"]:
        ...
        assert field.get_value() == (2 ** field.get_n_bits() - 1), (
            f"Failing for access {acs}"
        )
        assert field.get_response() == uvm_resp_t.PASS_RESP, (
            f"Failing as default status is not PASS_RESP is {field.get_response()}"
        )
        expected = True
        assert value == expected, f"Expected {expected} got {value} for for access {acs}"
...
def test_reg_field_field_predict_write_set():
    ...
    for acs in ["WSRC", "WOS", "WS", "W0S", "W1SRC", "W0SRC", "W1S"]:
        ...
        if (
            field.get_access() in ["W0S", "W0SRC"]
        ):  ## anytime a bit is clear to 0 that bit is gonna be set to 1 if not already 1
            predicted_v = field.get_reset() | (
                ~local_rand_el & int("".join(["1"] * field.get_n_bits()), 2)
            )
        elif field.get_access() in [
            "W1SRC",
            "W1S",
        ]:  ## anytime a bit is set to 1 that bit is gonna be set to 1 if not already 1
            predicted_v = field.get_reset() | (
                local_rand_el & int("".join(["1"] * field.get_n_bits()), 2)
            )
        else:
...
def test_reg_field_field_predict_write_clear():
    ...
    for acs in ["WOC", "WC", "W1C", "W1CRS", "W0C", "W0CRS", "WCRS"]:
        ...
        if (
            field.get_access() in ["W0C", "W0CRC"]
        ):  ## anytime a bit is clear to 0 that bit is gonna be set to 0 if not already 0
            predicted_v = field.get_reset() & (
                local_rand_el & int("".join(["1"] * field.get_n_bits()), 2)
            )
        elif field.get_access() in [
            "W1CRC",
            "W1C",
        ]:  ## anytime a bit is set to 1 that bit is gonna be set to 0 if not already 0
            predicted_v = field.get_reset() & (
                ~local_rand_el & int("".join(["1"] * field.get_n_bits()), 2)
            )
        else:
...
def test_reg_field_field_predict_TOGGLE():
    ...
    for acs in ["W1T", "W0T"]:
        ...
        if (
            "0" in field.get_access()
        ):  ## anytime a bit is cleared to 0 that bit is gonna be set to 1 or 0 toggling
            predicted_v = field.get_reset() ^ (
                ~local_rand_el & int("".join(["1"] * field.get_n_bits()), 2)
            )
