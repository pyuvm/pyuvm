...
def test_reg_get_name():
    for elem in range(1, 32):
        ...
        with pytest.warns(
            DeprecationWarning, match="The 'get_reg_size' method is deprecated, use 'get_n_bits' instead"):
            assert reg.get_reg_size() == elem
        with pytest.warns(DeprecationWarning, match="The 'check_err_list' method is deprecated"):
            reg.check_err_list()
