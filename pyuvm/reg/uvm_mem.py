...
class uvm_mem(uvm_object):
    ...
    def configure(self, parent: uvm_reg_block, hdl_path: str = "") -> None:
        ...
        if self._access not in ["RW", "RO"]:
            logger.error(f"Memory '{self.get_full_name()}' can only be 'RW' "
    ...
    def set_offset(self,
        ...
        if len(self._maps) > 1 and not map:
            logger.error(f"Set offset requires a map when memory "
        ...
        for local_map in self._maps:
            ...
            while parent_map:
                if parent_map == map:
                    return local_map
                parent_map = parent_map.get_parent_map()
        logger.warning(f"Memory '{self.get_full_name()}' is not contained "
                       f"within map '{map.get_full_name()}'")

    def get_default_map(self) -> uvm_reg_map | None:
        if not self._maps:
            logger.warning(f"Memory '{self.get_full_name()}' is not "
    ...
    def get_access(self, map: uvm_reg_map = None) -> str:
        ...
        if rights == "RW":
            ...
            if self._access in ["RW", "WO"]:
                ...
                logger.error(f"RO memory '{self.get_full_name()}' restricted "
                             f"to WO in map '{local_map.get_full_name()}'")
            else:
                logger.error(f"Memory '{self.get_full_name()}' has invalid "
    ...
    def get_vreg_by_name(self, name: str) -> uvm_vreg | None:
        for vreg in self.get_virtual_registers():
            if vreg.get_name() == name:
                return vreg
        logger.warning(f"Unable to find virtual register '{name}' in memory "
    ...
    def get_vfield_by_name(self, name: str) -> uvm_vreg_field | None:
        for field in self.get_virtual_fields():
            if field.get_name() == name:
                return field
        logger.warning(f"Unable to find virtual field '{name}' in memory "
                       f"'{self.get_full_name()}'")
    ...
    def get_addresses(
        ...
        if offset >= self._size:
            logger.warning(f"Offset 0x{offset:X} lies outside of memory "
                f"'{self.get_name()}' which has size 0x{self._size:X}")
            return -1, list()
        local_map = self.get_local_map(map)
        if not local_map:
            map_name = "None" if not map else map.get_full_name()
            logger.warning(f"Memory '{self.get_name()}' not found in map "
                           f"'{map_name}'")
            return -1, list()
        ...
        if map_info.unmapped:
            ...
            logger.warning(f"Memory '{self.get_name()}' is unmapped in map "
                           f"'{map_name}'")
            return -1, list()
