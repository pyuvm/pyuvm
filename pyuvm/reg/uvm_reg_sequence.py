...
class uvm_reg_frontdoor(uvm_reg_sequence):
    ...
    def atomic_unlock(self):
        if not self._atomic.locked():
            logger.warning(f"Attempt to unlock frontdoor "
                f"'{self.get_full_name()}' when it wasn't locked!")
    ...
    async def start(self,
                    ...
                    this_priority: int = -1,
                    call_pre_post: bool = True) -> None:
        # TODO: implement guard check. See official UVM implementation
        super().start(
            sequencer, parent_sequence, this_priority, call_pre_post
        )
