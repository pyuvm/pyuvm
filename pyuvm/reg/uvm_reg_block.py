...
class uvm_reg_block(uvm_object):
    ...
    _root_names: list[str] = list()
    _roots: list[uvm_reg_block] = list()
    _enable_reg_lookup_cache: bool =False
    _reg_block_registry: dict[str, uvm_reg_block] = dict()
    ...
    def set_default_map(self, map: uvm_reg_map) -> None:
        if map not in self._maps:
            logger.error(f"Map '{map.get_name()}' does not exist in block")
            return
    ...
    def _add_block(self, blk: uvm_reg_block) -> None:
        ...
        if uid in self._blks:
            logger.error(f"Subblock '{blk.get_name()}' has already been "
                         f"registered with block '{self.get_name()}'")
            return
        self._blks[uid] = blk
    ...
    def _add_map(self, map: uvm_reg_map) -> None:
        ...
        if map in self._maps:
            logger.error(f"Map '{map.get_name()}' already exists in "
    ...
    def _add_register(self, reg: uvm_reg) -> None:
        ...
        if uid in self._regs:
            logger.error(f"Register '{reg.get_name()}' has already been "
    ...
    def lock_model(self) -> None:
        ...
        if self.get_parent():
            ...
            if max_size > UVM_REG_DATA_WIDTH:
                raise UVMFatalError("Register model requires that "
            ...
            while names:
                ...
                _ = names.pop(0)
                if count > 1:
                    logger.error(f"There are {count} root register models "
                        "named '{name}'. The names of the root register "
    ...
    def find_blocks(name: str,
                    ...
                    accessor: uvm_object = None) -> list[uvm_reg_block]:
        raise NotImplementedError
    ...
    def get_block_by_name(self, name: str) -> uvm_reg_block | None:
        ...
        for blk in self.get_blocks(uvm_hier_e.UVM_HIER):
            ...
            if block:
                return block
        logger.warning(f"Unable to locate block '{name}' in block "
                       f"'{self.get_full_name()}'")
    ...
    def get_map_by_name(self, name: str) -> uvm_reg_map:
        for map in self.get_maps():
            if map.get_name() == name:
                return map
        logger.warning(f"Unable to locate map '{name}' in block "
                       f"'{self.get_full_name()}'")
    ...
    def get_reg_by_name(self, name: str) -> uvm_reg | None:
        for reg in self.get_registers(uvm_hier_e.UVM_HIER):
            if reg.get_name() == name:
                return reg
        logger.warning(f"Unable to locate register '{name}' in block "
                       f"'{self.get_full_name()}'")
    ...
    def get_field_by_name(self, name: str) -> uvm_reg_field:
        for field in self.get_fields(uvm_hier_e.UVM_HIER):
            if field.get_name() == name:
                return field
        logger.warning(f"Unable to locate field '{name}' in block "
                       f"'{self.get_full_name()}'")
    ...
    def get_mem_by_name(self, name: str) -> uvm_mem | None:
        for mem in self.get_memories(uvm_hier_e.UVM_HIER):
            if mem.get_name() == name:
                return mem
        logger.warning(f"Unable to locate memory '{name}' in block "
                       f"'{self.get_full_name()}'")
    ...
    def get_vreg_by_name(self, name: str) -> uvm_vreg | None:
        for vreg in self.get_virtual_registers(uvm_hier_e.UVM_HIER):
            if vreg.get_name() == name:
                return vreg
        logger.warning(f"Unable to locate virtual register '{name}' in block "
                       f"'{self.get_full_name()}'")
    ...
    def get_vfield_by_name(self, name: str) -> uvm_vreg_field:
        for vfield in self.get_virtual_fields(uvm_hier_e.UVM_HIER):
            if vfield.get_name() == name:
                return vfield
        logger.warning(f"Unable to locate virtual field '{name}' in block "
                       f"'{self.get_full_name()}'")
    ...
    def set_lock(self, v: bool = None) -> None:
        if v is None:
            warnings.warn("The 'set_lock' method is changed. Please use "
