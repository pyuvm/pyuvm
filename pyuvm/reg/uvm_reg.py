...
class uvm_reg(uvm_object):
    ...
    def __init__(self,
        ...
        super().__init__(name)
        if n_bits < 1:
            logger.error(f"Register '{self.get_name()}' cannot have 0 bits")
            n_bits = 1
        ...
        self._is_locked_by_field: bool = False
        self._atomic_cnt: int = 0
        ...
        for arg, val in kwargs.items():
            if arg == "reg_width":
                self._n_bits = val
                warnings.warn(
                    "The 'reg_width' argument is deprecated, use "
                    "'n_bits' instead", DeprecationWarning, 2)
            else:
    ...
    def configure(self,
                  ...
                  **kwargs) -> None:
        # TODO: remove backward compatibility
        if ((not isinstance(regfile_parent, uvm_reg_file) and
             regfile_parent is not None) or 'addr' in kwargs):
            warnings.warn(
                "The 'configure' definition as changed, the address should be "
                "specified when the register is added to a map"
                "'blk_parent' instead", DeprecationWarning, 2)
            for arg, val in kwargs.items():
                if arg == "addr":
                    self._addr = val
                else:
                    raise SyntaxError(f"Unknown argument {arg}")
            if (not isinstance(regfile_parent, uvm_reg_file) or
                regfile_parent is not None):
                    self._addr = regfile_parent
        # END: backward compatibility

        if blk_parent is None:
            logger.error("uvm_reg.configure() called without a parent block "
        ...
        if self._addr is not None:
            try:
                ...
                logger.error("Calling build() when not implemented by user"
        ...
        try:
            if (throw_error_on_write is None or
                throw_error_on_write is not None):
                    pass
        except NameError:
    ...
    def set_offset(self,
        ...
        if len(self._maps) > 1 and not map:
            logger.error(f"'set_offset' requires a map when register "
    ...
    def _add_field(self, field: uvm_reg_field) -> None:
        ...
        # NOTE: Check if any fields are to large for the register
        for f in self._fields:
            if f.get_lsb_pos() + f.get_n_bits() > self.get_n_bits():
                logger.error(
                    f"Field {f.get_name()} is too large for register "
                    f"{self.get_name()}")
    ...
    def add_map(self, map: uvm_reg_map) -> None:
        if map in self._maps:
            logger.error(f"Map '{map.get_name()}' is already added")
        else:
        ...
        for local_map in self._maps:
            ...
            while parent_map is not None:
                if parent_map == map:
                    return local_map
                parent_map = parent_map.get_parent_map()
        logger.warning(f"Register {self.get_full_name()} is not contained "
                       f"within map {map.get_full_name()}")
        return None

    def get_default_map(self) -> uvm_reg_map | None:
        if len(self._maps) == 0:
            logger.warning(f"Register {self.get_full_name()} is not "
                           "registered with any map")
            return None
        if len(self._maps) == 1:
            return self._maps[0]
    ...
    def get_addresses(
        ...
        if map_info.unmapped:
            ...
            logger.warning(f"Register '{self.get_name()}' is unmapped in map "
                           f"'{map_name}'")
    ...
    async def write(self,
        ...
        if not isinstance(path, uvm_door_e):
            warnings.warn("The 'write' arguments has changed in order to "
                "match IEEE 1800.2 standard.", DeprecationWarning, 2)
            _map = path
            path = map
            map = _map
        if 'check' in kwargs:
            warnings.warn("The 'check' arguments is deprecated",
    ...
    async def read(self,
                   ...
                   fname: str = "",
                   lineno: int = 0,
                   **kwargs) -> tuple[uvm_status_e, uvm_reg_data_t]:
        # TODO: remove backward compatibility
        if not isinstance(path, uvm_door_e):
            warnings.warn("The 'read' arguments has changed in order to "
                "match IEEE 1800.2 standard.", DeprecationWarning, 2)
            _map = path
            path = map
            map = _map
    ...
    def _check_access(
        self,
        rw: uvm_reg_item
    ) -> tuple[bool, uvm_reg_map_info | None]:
        map_info = None
    ...
    async def _do_write_backdoor(self,
                           rw: uvm_reg_item,
                           map_info: uvm_reg_map_info) -> None:
        raise NotImplementedError

    async def _do_write_frontdoor(self,
                            rw: uvm_reg_item,
                            map_info: uvm_reg_map_info) -> None:
    ...
    async def _do_read_backdoor(self,
                          rw: uvm_reg_item,
                          map_info: uvm_reg_map_info) -> None:
        raise NotImplementedError

    async def _do_read_frontdoor(self,
                           rw: uvm_reg_item,
                           map_info: uvm_reg_map_info) -> None:
        local_map = rw.get_local_map()
        system_map = local_map.get_root_map()
        ...
        self._set_is_busy(False)
        if system_map.get_auto_predict():
            if (local_map.get_check_on_read() and
                rw.get_status() != uvm_status_e.UVM_NOT_OK):
    ...
    def do_predict(self,
                   rw: uvm_reg_item,
                   kind: uvm_predict_e,
                   be: uvm_reg_byte_en_t = -1) -> None:
        reg_value = rw.get_value()
        self._fname = rw.get_fname()
        self._lineno = rw.get_line()
        if rw.get_status() == uvm_status_e.UVM_IS_OK:
            if self.is_busy() and kind == uvm_predict_e.UVM_PREDICT_DIRECT:
                logger.warning("Trying to predict value of register "
                    f"'{self.get_name()}' while it is being accessed")
                return
            ...
            logger.warning("Status UVM_NOT_OK; skip prediction.")
...
# `uvm_register_cb(uvm_reg, uvm_reg_cbs)
    ...
    def get_reg_size(self) -> int:
        warnings.warn("The 'get_reg_size' method is deprecated, use "
                      "'get_n_bits' instead", DeprecationWarning, 2)
        return self.get_n_bits()

    @property
    def n_bits(self) -> int:
        warnings.warn("The 'n_bits' attribute is deprecated, use the method "
                      "'get_n_bits' instead", DeprecationWarning, 2)
        return self.get_n_bits()

    def check_err_list(self) -> None:
        warnings.warn("The 'check_err_list' method is deprecated",
                      DeprecationWarning, 2)

    def _set_lock(self) -> None:
        warnings.warn("The '_set_lock' method is deprecated",
                      DeprecationWarning, 2)

    def set_desired(self, value):
        warnings.warn("The 'set_desired' method is deprecated, use 'set' "
                    "instead", DeprecationWarning, 2)
        self.set(value)

    def get_desired(self):
        warnings.warn("The 'get_desired' method is deprecated, use 'get' "
                    "instead", DeprecationWarning, 2)
        return self.get()

    def get_access_policy(self):
        warnings.warn("The 'get_access_policy' method is deprecated, use "
                      "'get_rights' instead", DeprecationWarning, 2)
