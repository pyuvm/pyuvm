...
class uvm_reg_map(uvm_object):
    _backdoor: uvm_reg_backdoor = None
        ...
        for reg, reg_info in self._regs_info.items():
            ...
            if not reg_info.unmapped:
                ...
                for addr in reg_addrs:
                    if (addr in self._regs_by_offset and
                        root_map._regs_by_offset.get(addr) is not reg):
                            ...
                            if reg_access == "RO" and other_reg_access == "WO":
                                ...
                                logger.warning(f"In map '{self.get_full_name()}' "
                                    f"register '{reg.get_full_name()}' maps "
                                    "to the same address as register "
                                    f"'{other_reg.get_full_name()}': 0x{addr:X}")
                    else:
                        root_map._regs_by_offset[addr] = reg
    ...
    def configure(self,
        ...
        if n_bytes is None:
            warnings.warn("n_bytes not set, assuming 4 bytes",
                          DeprecationWarning, 2)
            n_bytes = 4
        if endian is None:
            warnings.warn("endian not set, assuming little endian",
                          DeprecationWarning, 2)
    ...
    def add_reg(self,
                ...
                rights: str = "RW",
                unmapped: bool = False,
                frontdoor: uvm_reg_frontdoor = None) -> None:
        if rg in self._regs_info:
            logger.error(f"Register '{rg.get_name()}' has already been added"
                         f"to map '{self.get_full_name()}'")
        if rg.get_parent() != self.get_parent():
            logger.error(f"Register '{rg.get_name()}' may not be added to "
                         f"the address map '{self.get_full_name()}' : they  "
    ...
    def set_sequencer(self,
        ...
        if not adapter:
            logger.info(
                f"Adapter not specified for map '{self.get_full_name()}'. "
                "Accesses via this map will send abstract 'uvm_reg_item' "
                f"items to sequencer '{sequencer.get_full_name()}'")
    ...
    def _set_reg_offset(self,
        ...
        if reg not in self._regs_info:
            logger.error(f"Cannot modify offset of register "
                         f"'{reg.get_full_name()}' in address map "
                         f"'{self.get_full_name()}' register is not "
                ...
                for addr in info.addr:
                    if addr not in root_map._regs_by_offset_wo:
                        ...
                        if root_map._regs_by_offset[addr] is reg:
                            root_map._regs_by_offset[addr] = root_map._regs_by_offset_wo[addr]
                            # TODO: callbacks
        ...
        if not unmapped:
            reg_access = reg._get_fields_access(self)
            bus_width, addrs = self.get_physical_addresses(
                offset, 0, reg.get_n_bytes())
            for addr in addrs:
                if (
                    addr in root_map._regs_by_offset and
                    root_map._regs_by_offset.get(addr) is not reg
                ):
                    ...
                    if reg_access == "RO" and reg2_access == "WO":
                        ...
                        logger.warning(f"In map '{self.get_full_name()}' "
                            f" register '{reg.get_full_name()}' maps to same "
                            f"address as register "
                            f"'{root_map._regs_by_offset[addr].get_full_name()}' "
                            f": 0x{addr:X}")
                else:
                    root_map._regs_by_offset[addr] = reg
                ...
                for range in root_map._mems_by_offset.keys():
                    if addr >= range.min and addr <= range.max:
                        logger.warning(f"In map '{self.get_full_name()}' "
                            f"register '{reg.get_full_name()}' "
                            "overlaps with address range of memory"
                            f"'{root_map._mems_by_offset[range].get_full_name()}' "
                            f": 0x{addr:X}")
            info.addr = addrs
        if unmapped:
            info.offset = -1
    ...
    def get_submaps(
        self,
        hier: uvm_hier_e = uvm_hier_e.UVM_HIER
    ) -> list[uvm_reg_map]:
        submaps = list()
    ...
    def get_memories(self,
                     hier: uvm_hier_e = uvm_hier_e.UVM_HIER) -> list[uvm_mem]:
        memories = list()
        if hier == uvm_hier_e.UVM_HIER:
            for submap in self._submaps:
    ...
    def get_virtual_registers(
            self,
            hier: uvm_hier_e = uvm_hier_e.UVM_HIER) -> list[uvm_vreg]:
    ...
    def get_reg_map_info(self, rg: uvm_reg,
                         error: bool = True) -> uvm_reg_map_info | None:
        if rg not in self._regs_info:
            if error:
                logger.error(f"Register '{rg.get_name()}' not in map "
                             f"'{self.get_full_name()}'")
            return
        map_info = self._regs_info[rg]
        if not map_info.is_initialized:
            logger.warning(f"Map '{self.get_full_name()}' does not seem to "
    ...
    def get_reg_by_offset(self,
                          offset: uvm_reg_addr_t,
                          read: bool = True) -> uvm_reg | None:
        if not self.get_parent().is_locked():
            logger.error(
                "Cannot get register by offset : block "
                f"'{self.get_parent().get_full_name()}' is not locked")
            return None
    ...
    async def do_bus_write(self,
        ...
        reg = rw.get_element()
        _, addrs, _ = self._get_physical_addresses_to_map(
            self._regs_info[reg].offset,
            0x0,
            reg.get_n_bytes(),
            None,
            None
    ...
    def _get_bus_info(
        ...
        if kind == uvm_elem_kind_e.UVM_MEM:
            ...
            if not mem:
                raise UVMFatalError(
                    f"uvm_reg_item 'elemend_kind' is UVM_MEM but 'element' "
            ...
            if not reg:
                raise UVMFatalError(
                    f"uvm_reg_item 'elemend_kind' is UVM_REG but 'element' "
            ...
            if not field:
                raise UVMFatalError(
                    f"uvm_reg_item 'elemend_kind' is UVM_FIELD but 'element' "
    ...
    def _get_physical_addresses_to_map(
        ...
        if map is not parent_map:
            ...
            laddr = lbase_addr + base_addr * self.get_addr_unit_bytes() / map.get_addr_unit_bytes()
            lb = (base_addr * self.get_addr_unit_bytes()) % map.get_addr_unit_bytes()
            byte_offset += lb
            # recursive call one level up
            return map._get_physical_addresses_to_map(laddr, 0, n_bytes+lb, parent_map, byte_offset)
        ...
        if mem_offset:
            if mem and mem.get_n_bytes() > self.get_addr_unit_bytes():
                lbase_addr2 = base_addr + mem_offset * mem.get_n_bytes() / self.get_addr_unit_bytes()
                byte_offset += mem_offset * mem.get_n_bytes() % self.get_addr_unit_bytes()
            else:
        ...
        if endian == uvm_endianness_e.UVM_LITTLE_ENDIAN:
            ...
            for i, _ in enumerate(local_addr):
                local_addr[i] = lbase_addr2 + (len(local_addr) - i - 1) * bus_width / self.get_addr_unit_bytes()
        elif endian in (uvm_endianness_e.UVM_LITTLE_FIFO, uvm_endianness_e.UVM_BIG_FIFO):
            local_addr = lbase_addr2 * n_addrs
        else:
            logger.error("Map has no specified endianness. Cannot access "
                         f"'{n_bytes}' bytes register via its '{bus_width}' "
                         f"byte '{self.get_full_name()}' interface")
    ...
    def get_offset(self) -> uvm_reg_addr_t:
        warnings.warn("The 'get_offset' method is deprecated, use "
                      "'get_base_addr' instead", DeprecationWarning, 2)
        return self.get_base_addr()

    def set_adapter(self, adapter) -> None:
        warnings.warn("The 'set_adapter' method is deprecated, use "
            "'set_sequencer(seqr, adapter)' instead", DeprecationWarning, 2)
        self._adapter = adapter
