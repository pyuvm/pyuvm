...
if TYPE_CHECKING:
    from pyuvm import uvm_reg_item, uvm_reg_map, uvm_sequence_base
...
class uvm_reg_field(uvm_object):
    ...
    _max_size: int
    _policy_names: set[str] = _PREDEFINED_POLICIES
    ...
    def configure(self,
        ...
        if has_reset is None:
            warnings.warn("The 'has_reset' argument must be "
                          "specified", DeprecationWarning, 2)
        ...
        # TODO: Remove backward compatibility check
        if is_rand is None:
            warnings.warn("The 'is_rand' argument must be "
                          "specified", DeprecationWarning, 2)
            is_rand = False
        # TODO: Remove backward compatibility check
        if individually_accessible is None:
            warnings.warn("The 'individually_accessible' argument must be "
                          "specified", DeprecationWarning, 2)
        ...
        for arg, val in kwargs.items():
            if arg == 'is_volatile':
                warnings.warn("The 'is_volatile' argument is deprecated, "
                              "use 'volatile' instead", DeprecationWarning, 2)
                self._volatile = val
            else:
        ...
        if self._access not in uvm_reg_field._policy_names:
            ...
            logger.error(f"Access policy '{self._access}' for field "
                         f"'{self.get_full_name()}' is not a defined field. "
                         f"setting to 'NOACCESS'")
            self._access = "NOACCESS"

        if self._access in ["RO", "RC", "RS", "WC", "WS", "W1C", "W1S", "W1T",
                           "W0C", "W0S", "W0T", "W1SRC", "W1CRS", "W0SRC",
                           "W0CRS", "WSRC", "WCRS", "WOC", "WOS"]:
            is_rand = False
    ...
    def set_access(self, mode: str) -> str:
        ...
        if self._access not in uvm_reg_field._policy_names:
            logger.error(f"Access policy '{self._access}' is not a defined "
                         "field access policy")
    ...
    def set_rand_mode(self, rand_mode: bool) -> None:
        try:
            ...
            if rand_mode is True:
                raise NotImplementedError("Randomization is not supported "
                                          f"for type '{type(self.value)}'")
    ...
    def get_access(self, map: uvm_reg_map = None) -> str | None:
        ...
        if self._parent.get_rights(map) in ["RW"]:
            ...
            if access in ["RW", "RO", "WC", "WS", "W1C", "W1S", "W1T",
                          "W0C", "W0S", "W0T", "W1"]:
                return "RO"
            ...
            logger.warning(f"Register '{self._parent.get_full_name()}' "
                f"containing field '{self.get_name()}' is mapped in map "
                f"'{map.get_full_name()}' with unknown access rights "
                f"'{self._parent.get_rights(map)}'")
        return "NOACCESS"
    ...
    def set_field(self, value: int) -> None:
        warnings.warn("The 'set_field' method is deprecated, "
            "use 'set' instead", DeprecationWarning, 2)
    ...
    def get_mirrored_value(self,
        ...
        if self.is_volatile():
            logger.warning("Mirrored value returned for volatile field "
                           f"'{self.get_full_name()}', register "
    ...
    def do_predict(self,
        ...
        if kind == uvm_predict_e.UVM_PREDICT_WRITE:
            if (
                rw.get_door() == uvm_door_e.UVM_FRONTDOOR or
                rw.get_door() == uvm_door_e.UVM_PREDICT
            ):
                field_value = self._predict(
                    self._mirrored, field_value, rw.get_map()
                )
            self.written = True
            ...
            if self.parent.is_busy():
                logger.warning("Trying to predict value of field "
                               f"'{self.get_name()}' while register "
                               f"'{self.parent.get_name()}' is being "
                               "accessed."
                )
    ...
    def field_lock(self) -> None:
        warnings.warn("The 'field_lock' method is deprecated",
                      DeprecationWarning, 2)
        pass

    def get_value(self) -> uvm_reg_data_t:
        warnings.warn("The 'get_value' method is deprecated, use 'get' "
