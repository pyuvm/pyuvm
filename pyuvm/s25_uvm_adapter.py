...
class uvm_reg_adapter(uvm_object):
    ...
    def __init__(self, name="uvm_reg_adapter"):
        ...
        #    if the bus protocol supports byte enables.
        self.byte_enable = True
        self.parent_sequence = uvm_sequence
        self.reg_item = uvm_sequence_item

    #    Function -- reg2bus
    #    Extensions of this class ~must~ implement
    #    this method to convert the specified
    #    <uvm_reg_bus_op> to a corresponding <uvm_sequence_item>
    #    subtype that defines the bus transaction.
    #    The method must allocate a new bus-specific <uvm_sequence_item>,
    #    assign its members from
    #    the corresponding members from the given generic
    #    ~rw~ bus operation, then return it.
    def reg2bus(rw: uvm_reg_bus_op):
        pass

    #    Function -- bus2reg
    #    Extensions of this class ~must~ implement
    #    this method to copy members of the given
    #    bus-specific ~bus_item~ to corresponding
    #    members of the provided ~bus_rw~ instance.
    #    Unlike <reg2bus>, the resulting transaction
    #    is not allocated from scratch. This is to
    #    accommodate applications where the bus
    #    response must be returned in the original request.
...
#            rw.kind   = UVM_WRITE;
#        else:
#            assert(0,"bus2reg -- Wrong operation type"
#                     "used for uvm_reg_bus_op")
#        rw.addr      = apb.addr;
#        rw.data      = apb.data;
#        rw.status    = UVM_IS_OK;
#
