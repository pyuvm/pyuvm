...
class uvm_reg(uvm_object):
    ...
    def _add_field(self, field):
        ...
        if (field in self._fields):
            error_out(self._header, f"_add_field: Fields {field.get_name()} is already added")
    ...
    async def write(self, value, map: uvm_reg_map, path: path_t, check: check_t) -> uvm_resp_t:
        ...
        if (self._op_in_progress is False):
            ...
            uvm_fatal(self._header, "write cannot perform an operation while another is in progress")
        # Return from Task
        return status
    ...
    async def read(self, map: uvm_reg_map, path: path_t, check: check_t):
        ...
        if (self._op_in_progress is False):
            ...
            uvm_fatal(self._header, "read cannot perform an operation while another is in progress")
        # Return from Task
