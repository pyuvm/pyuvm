...
class uvm_reg(uvm_object):
    ...
    def __init__(self, name="uvm_reg", reg_width=32):
        ...
        # there will be no difference between read and write
        # we cannot read and write from to the same register at tsame time
        self._op_in_progress = False

    # configure
    def configure(self, parent: uvm_reg_block,
                  address: uvm_reg_addr_t,
                  hdl_path: str,
    ...
    # setting the desired value, if this one is set we can avoid
    # using the Value in write
    ...
    def _add_field(self, field):
        ...
        if (field in self._fields):
            error_out(self._header, """_add_field: Fields
                      {} is already added""".format(field.get_name()))
        ...
        if (self._width < self._sum):
            error_out(self._header, "_add_field: Fields"
                                    "{} doesn't fit into a {} bits register".
                                    format(field.get_name(), self._width))
            self._add_error(
                uvm_reg_error_decoder.FIELD_DOESNT_FIT_INTO_REG.name)
        # - field doesn't overlap with any other field
        if (len(self._fields) > 1):
            field_index = self._fields.index(field) - 1
            if field.get_lsb_pos() <= \
                    self._fields[field_index].get_msb_pos():
                error_out(self._header,
                          "_add_field: Fields {} overlap with field {}".
                          format(field.get_name(), self._fields[field_index].
                                 get_name()))
                self._add_error(uvm_reg_error_decoder.
                                FIELD_OVERLAPPING_ERROR.name)
    ...
    def predict(self, value, direction: access_e):
        for f in self.get_fields():
            f.field_predict((value >> f.get_lsb_pos()),
                            path_t.FRONTDOOR,
                            direction)
    ...
    def get_mirrored_value(self):
        ...
        for f in self.get_fields():
            self._mirrored |= (f.get_value() << f.get_lsb_pos())
            print(f"""f.get_value() {f.get_value()} -- f.get_lsb_pos()
                   {f.get_lsb_pos()} -- self._mirrored {self._mirrored }""")
    ...
    def get_desired(self):
        self._desired = 0
        for f in self._fields:
            self._desired |= (f.get() << f.get_lsb_pos())
    ...
    async def write(self, value, map: uvm_reg_map,
        ...
        # We check instead if the map is set and if only one
        # exists (multiple access)
        # The map the register belongs to should not be unique
        # could easily be a list of MAPS
        # every map correspond a separate unique HW interface (BUS) to
        # the specific target register
        ...
        if (self._op_in_progress is False):
            ...
            raise UVMFatalError("uvm_reg -- write -- cannot "
                                "perform an operation while "
    ...
    async def read(self, map: uvm_reg_map, path: path_t, check: check_t):
        ...
        # register exists in the map
        # (redundant check) since the register is directly taken from the MAP
        # We check instead if the map is set and if only
        # one exists (multiple access)
        ...
        if (self._op_in_progress is False):
            ...
            raise UVMFatalError("uvm_reg -- read -- cannot perform an "
