...
from pyuvm.s17_pyuvm_reg_enumerations import uvm_reg_policy_t
from pyuvm.s24_pyuvm_reg_includes import uvm_resp_t, path_t
from pyuvm.s24_pyuvm_reg_includes import predict_t, check_t, uvm_fatal
from pyuvm.s24_pyuvm_reg_includes import error_out, access_e
...
class uvm_reg_field(uvm_object):
    ...
    # configure
    def configure(self, parent: uvm_reg, size: int, lsb_pos: int, access: str, is_volatile: bool, reset: int):
        self._parent = parent
        self._size = size
        ...
        # Check if size is 0
        if (self._size == 0):
            uvm_fatal(self._header, "Size of a filed cannot be 0 MINIMUN allowed is 1")
        ...
        if (self._reset >= (2**self._size)):
            uvm_error(self._header, f"Reset value for REG feild : {self._name} is [{self._reset}] is beyond the MAX valoue given by the size [{((2**self._size)-1)}]")
        # Add field
    ...
    def set_access(self,access_value):
        if (not isinstance(access_value, str)):
            ...
            self._add_error(uvm_reg_field_error_decoder.ACCESS_TYPE_NEEDS_TO_BE_A_STRING)
        else:
    ...
    def predict_based_on_read(self, value):
        ...
        if self.get_access() in ["RC", "WRC", "WSRC", "W1SRC", "W0SRC"]:
            ...
            self._add_error(uvm_reg_field_error_decoder.WRONG_ACCESS_FOR_PREDICT_READ)
            uvm_error(self._header, "Wrong Access set on predict_based_on_read")
    ...
    def predict_response(self, value, path: path_t, direction: access_e):
        ...
        if ((direction == access_e.UVM_WRITE) & (self.get_access() in ["RO", "RW", "RC", "RS"]) & (path == path_t.FRONTDOOR)):
            self.set_response(uvm_resp_t.PASS_RESP if (self._error_on_write is False) else uvm_resp_t.ERROR_RESP)
        elif ((direction == access_e.UVM_READ) & (self.get_access() in ["WO", "WOC", "WOS", "WO1", "NOACCESS", "W1", "W1T", "W0T", "WC", "WS", "W1C", "W1S", "W0C", "W0S"]) & (path == path_t.FRONTDOOR)):
    ...
    def field_predict(self, value, path: path_t, direction: access_e):      
        ...
        if ((direction == access_e.UVM_WRITE) & (path == path_t.FRONTDOOR) & (self._prediction in [predict_t.PREDICT_DIRECT, predict_t.PREDICT_WRITE])):
            self.predict_based_on_write(value)
        # READ prediction and prediction is ether on read and Direct
        elif ((direction == access_e.UVM_READ) & (path == path_t.FRONTDOOR) & (self._prediction in [predict_t.PREDICT_DIRECT, predict_t.PREDICT_READ])):
            ...
            uvm_error(self._header, "Wrong combination of PATH - PREDICTION TYPE and DIRECTION on pyuvm_field -- field_predict function")
