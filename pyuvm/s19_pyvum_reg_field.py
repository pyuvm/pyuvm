...
class pyuvm_reg_field(uvm_object):
    ...
    ## configure
    def configure(self, parent, size: int, lsb_pos: int, access: str, is_volatile: bool, reset: int):
        self._parent        = parent
        self._size          = size
        ...
        ## Check if size is 0
        if(self._size == 0):
            raise UVMFatalError(f"{self._header} Size of a filed cannot be 0 MINIMUN allowed is 1")
        ...
        if(self._reset >= (2**self._size)):
            error_out(self._header,f"Reset value for REG feild : {self._name} is [{self._reset}] is beyond the MAX valoue given by the size [{((2**self._size)-1)}]")
        ## Add field
    ...
    def field_set(self,value: int):
        ...
        ## Ideally the set value should be checked against the parent register being accessed.
        ## if the parent is under WRITE there should be no set called. Not yet implemenmted.
        ##
        ##
